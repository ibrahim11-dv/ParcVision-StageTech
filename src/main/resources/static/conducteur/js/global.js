...
function initStatusToggler() {
    ...
    const control = document.getElementById('statusControl'); // New big button in mission page
    
    const saved = parseInt(localStorage.getItem('conducteur-status') || '0', 10);
    let idx = saved;

    function refreshUI() {
        if (badge) applyStatus(badge, idx);
        if (control) applyStatusControl(control, idx);
    }

    refreshUI();

    // The control element (big button) handles the click
    control?.addEventListener('click', () => {
        idx = (idx + 1) % STATUS_CYCLE.length;
        localStorage.setItem('conducteur-status', idx);
        refreshUI();
        showToast(`Statut mis à jour : ${STATUS_CYCLE[idx].label}`, 'info');
        document.dispatchEvent(new CustomEvent('statusChanged', { detail: STATUS_CYCLE[idx].cls }));
    });
    
    // Note: badge is no longer clickable by default, unless it is also the control
}
...
function applyStatusControl(el, idx) {
    const status = STATUS_CYCLE[idx];
    // Update the "big button" appearance
    el.className = `status-control-card ${status.cls}`;
    const iconEl = el.querySelector('.sc-icon i');
    const labelEl = el.querySelector('.sc-label');
    const descEl = el.querySelector('.sc-desc');
    
    const icons = {
        'status-en-mission': 'bi-signpost-2-fill',
        'status-disponible': 'bi-check-circle-fill',
        'status-conge':      'bi-calendar-event-fill',
        'status-suspendu':   'bi-pause-circle-fill'
    };
    
    const descs = {
        'status-en-mission': 'Vous effectuez actuellement un trajet officiel. Votre position est partagée en temps réel.',
        'status-disponible': 'Vous êtes prêt à recevoir de nouvelles missions et à être assigné à des trajets.',
        'status-conge':      'Vous êtes actuellement hors service ou en période de repos réglementaire.',
        'status-suspendu':   'Votre activité est temporairement suspendue. Veuillez consulter vos notifications.'
    };

    if (iconEl) iconEl.className = `bi ${icons[status.cls]}`;
    if (labelEl) labelEl.textContent = status.label;
    if (descEl) descEl.textContent = descs[status.cls];
}
